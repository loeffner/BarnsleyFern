...
const float FernProducer::m_prob[4] = 
         {0.01,   0.85,  0.07,  0.07};
 ...
 * Constructor
 */
FernProducer::FernProducer(Buffer<Point>& buffer, unsigned int id)
 :  Producer(buffer), m_id(id),  m_current({0.0, 0.0})
{ 
    dist = std::discrete_distribution<>({1, 85, 7, 7});
}
